...
async function update() {
  ...
  try {
    const [res] = await connection.execute(
      ...
      ['COMMUNICATION', 'Call Logs', '/calling/logs', 'Phone', 3]
    );
    ...
    if (roles.length > 0) {
      ...
      // get the new menu id
      const [menus] = await connection.execute('SELECT id FROM menus WHERE name = "Call Logs"');
      if (menus.length > 0) {
    ...
    console.log("Calling Menu added");
  } catch (err) {
    console.error(err);
  } finally {

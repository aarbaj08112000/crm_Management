  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <form onSubmit={handleAddRole} className="flex gap-4 items-end">
            <div className="flex-1">
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Role Name</label>
              <input
                type="text"
                required
                value={newRoleName}
                onChange={e => setNewRoleName(e.target.value)}
                className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
      ...
      <div className="flex gap-6 items-start">
        {/* Roles List */}
        <div className="w-64 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden shrink-0">
          <div className="p-4 border-b border-slate-100 bg-slate-50 dark:bg-slate-800">
            <h2 className="font-semibold text-slate-700 dark:text-slate-200">User Groups</h2>
          </div>
          <div className="divide-y divide-slate-100">
            {roles.map(role => (
              <button
                ...
                className={`w-full text-left px-4 py-3 transition-colors hover:bg-slate-50 dark:bg-slate-800 ${
                  selectedRole?.id === role.id ? 'bg-blue-50/50 text-blue-700 border-l-4 border-blue-600' : 'text-slate-600 dark:text-slate-300 border-l-4 border-transparent'
                }`}
              >
        ...
        {selectedRole && (
          <div className="flex-1 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-slate-100 bg-slate-50 dark:bg-slate-800 flex items-center justify-between">
                ...
                <tbody className="divide-y divide-slate-100">
                  {permissions.length === 0 ? (
                    ...
                    permissions.map(perm => (
                      <tr key={perm.menu_id} className="hover:bg-slate-50 dark:bg-slate-800/50 transition-colors">
                        <td className="py-3 px-4">
                          <div className="font-medium text-slate-800 dark:text-slate-100">{perm.menu_name || menus.find(m => m.id === perm.menu_id)?.name}</div>

...
export default function WhatsAppMessenger() {
  ...
  return (
    <div className="flex bg-gray-50 h-[calc(100vh-64px)] font-sans border-t border-gray-200">
      ...
      {/* LEFT PANEL */}
      <div className="w-[320px] lg:w-[350px] bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 flex flex-col flex-shrink-0">
        ...
        <div className="p-4 border-b flex justify-between items-center bg-white dark:bg-slate-900">
          <div>
            ...
            <select className="bg-transparent text-[13px] font-bold text-slate-700 dark:text-slate-200 outline-none w-full appearance-none pr-4 focus:ring-0 cursor-pointer ">
        ...
        <div className="p-4 border-b bg-white dark:bg-slate-900">
          <div className="relative group">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-100 border-transparent rounded-xl text-sm focus:outline-none focus:bg-white dark:bg-slate-900 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all text-slate-700 dark:text-slate-200 placeholder:text-slate-400 font-medium"
              value={searchQuery}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-50 dark:bg-slate-800">
          {conversations
            .filter(c => 
              !searchQuery || 
            ...
            return (
              <div 
                ...
                className={`flex p-4 border-b border-slate-100 cursor-pointer transition-all relative ${chat.id === activeChatId ? 'bg-white dark:bg-slate-900 shadow-[inset_4px_0_0_0_#10b981]' : 'hover:bg-white dark:bg-slate-900/60'}`}
              >
                <div className="relative flex-shrink-0 mr-4">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-bold shadow-sm ${chat.id === activeChatId ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-600 dark:text-slate-300'}`}>
                    {chat.initials}
                  </div>
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  <div className="flex justify-between items-center mb-1">
                    <h4 className={`text-[14px] truncate pr-2 ${chat.id === activeChatId ? 'text-emerald-950 font-bold' : 'text-slate-800 dark:text-slate-100 font-semibold'}`}>{chat.name}</h4>
                  ...
                  <p className={`text-[13px] truncate flex items-center gap-1 ${hasUnread && chat.id !== activeChatId ? 'text-slate-800 dark:text-slate-100 font-bold' : 'text-slate-500 font-medium'}`}>
      ...
      <div className="flex-1 flex flex-col bg-[#efeae2] min-w-[400px] relative" style={{ backgroundImage: 'url("https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png")', backgroundSize: 'contain', backgroundRepeat: 'repeat' }}>
        {/* Chat Header */}
        <div className="bg-white dark:bg-slate-900/80 backdrop-blur-md px-6 py-4 flex items-center justify-between border-b border-slate-200 dark:border-slate-700 z-10 sticky top-0 shadow-sm">
          {activeContact ? (
            <div className="flex items-center">
              ...
              <div>
                <h2 className="text-[15px] font-bold text-slate-800 dark:text-slate-100 leading-tight">{activeContact.name}</h2>
                ...
                <div className={`max-w-[290px] sm:max-w-md relative group`}>
                  ...
                  <div className={`shadow-sm relative ${
                    isAgent 
                      ...
                      : 'bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 rounded-2xl rounded-tl-sm border border-slate-100'
                  } ${isMedia ? 'p-1.5' : 'p-3.5'}`}>
                    ...
                    {msg.media_type === 'image' && msg.media_url ? (
                      ...
                      <div className="w-[260px] sm:w-[280px]">
                         <a href={msg.media_url} download target="_blank" rel="noopener noreferrer" className="flex items-center p-3 bg-black/5 rounded-lg hover:bg-black/10 transition-colors mb-1">
                           ...
                           <div className="ml-2 text-gray-500 bg-white dark:bg-slate-900 p-1.5 rounded-full shadow-sm hover:shadow">
                  ...
                  {isAgent && (
                     <button onClick={() => handleDeleteMessage(msg.id)} className="absolute top-1/2 -left-8 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity p-1.5 text-gray-400 hover:text-red-500 bg-white dark:bg-slate-900 rounded-full shadow hover:shadow-md" title="Delete Message">
        ...
        <div className="bg-white dark:bg-slate-900/80 backdrop-blur-md px-4 py-3 flex items-center gap-3 z-10 sticky bottom-0 border-t border-slate-200 dark:border-slate-700">
          <div className="relative" ref={emojiPickerRef}>
            <button 
               onClick={() => setShowEmojiPicker(!showEmojiPicker)}
               className="text-slate-400 hover:text-emerald-500 transition-colors p-2 cursor-pointer bg-slate-50 dark:bg-slate-800 rounded-full hover:bg-emerald-50"
               title="Emoji picker"
            >
          ...
          <button 
             ...
             className="text-slate-400 hover:text-emerald-500 transition-colors p-2 disabled:opacity-50 bg-slate-50 dark:bg-slate-800 rounded-full hover:bg-emerald-50"
             title="Attach Photo or Document"
          >
            <Paperclip className="w-5 h-5" />
          </button>

          <div className="flex-1 bg-slate-100 rounded-full px-5 shadow-inner border border-transparent focus-within:border-emerald-300 focus-within:bg-white dark:bg-slate-900 transition-all">
             <input 
               type="text"
               placeholder="Write a message..." 
               className="w-full outline-none text-[14px] bg-transparent py-3 font-medium text-slate-700 dark:text-slate-200 placeholder:text-slate-400"
               value={messageText}
      ...
      <div className="hidden w-[300px] lg:w-[320px] bg-white dark:bg-slate-900 border-l flex-col flex-shrink-0 z-20">
        ...
        {activeContact && (
          <div className="p-4 border-t bg-gray-50">
            <button className="w-full py-2 bg-white dark:bg-slate-900 border border-gray-200 text-gray-600 rounded text-sm font-medium hover:bg-gray-100 transition-colors shadow-sm cursor-pointer ">
              More Details
            </button>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all">
            ...
            <div className="px-6 py-4 bg-gray-50 border-t flex justify-end gap-3">
              <button onClick={() => setShowNewChatModal(false)} className="px-4 py-2 text-sm font-medium text-gray-600 bg-white dark:bg-slate-900 border rounded-lg hover:bg-gray-100">Cancel</button>
              <button 
      ...
      {showTemplateModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all">
            <div className="px-6 py-4 border-b bg-gray-50 flex justify-between items-center">
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <select 
                  ...
                  className="w-full border py-2.5 rounded-lg px-3 text-sm focus:border-green-500 outline-none bg-white dark:bg-slate-900 cursor-pointer "
                >
            ...
            <div className="px-6 py-4 bg-gray-50 border-t flex justify-end gap-3">
              <button 
                ...
                className="px-4 py-2 text-sm font-medium text-gray-600 bg-white dark:bg-slate-900 border rounded-lg hover:bg-gray-100"
              >
                Cancel
              </button>
              <button 
      ...
      {showSimulateModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="px-6 py-4 bg-gray-50 border-t flex justify-end gap-3">
              <button onClick={() => setShowSimulateModal(false)} className="px-4 py-2 text-sm font-medium text-gray-600 bg-white dark:bg-slate-900 border rounded-lg hover:bg-gray-100">Cancel</button>
              <button
      ...
      {deleteMessageId && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-[400px] max-w-[90%] overflow-hidden transform transition-all">
            <div className="p-6">
              ...
              <div className="flex justify-center gap-3">
                ...
                <button 
                  ...
                  className="px-8 py-2 text-sm font-medium text-gray-600 bg-white dark:bg-slate-900 border border-gray-300 rounded hover:bg-gray-50 transition-colors"
                >
                  Cancel
                </button>
              </div>

...
export default function EmailThreadModal({ enquiryId, enquiryName, enquiryEmail, initialSubject, onClose }) {
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isBold ? "bg-white dark:bg-[#121212] hover:bg-slate-50 dark:hover:bg-[#1C1C1C] border-slate-200 dark:border-[#27272A]" : "bg-slate-50/50 dark:bg-[#1A1A1A] hover:bg-slate-100/50 dark:hover:bg-[#1F1F1F] border-slate-100/60 dark:border-[#27272A]"
              )}
            >
              <div className="flex items-center gap-4 w-[220px] flex-shrink-0 pr-2">
                ...
                <span className={cn(
                  ...
                  isBold ? "text-slate-900 dark:text-slate-100 font-bold" : "text-slate-500 dark:text-slate-400 font-medium"
                )}>
                  {senderLabel}
                </span>
              </div>
              ...
              <div className="flex-1 min-w-0 flex items-center pr-4">
                <div className="truncate w-full text-sm">
                  <span className={cn(
                    isBold ? "text-slate-900 dark:text-slate-100 font-bold" : "text-slate-600 dark:text-slate-300 font-medium"
                  )}>
                    {thread.subject}
                  </span>
                  <span className="text-slate-400 dark:text-slate-500 mx-1.5">-</span>
                  <span className={cn(
                    isBold ? "text-slate-600 dark:text-slate-300 font-medium" : "text-slate-500 dark:text-slate-400"
                  )}>
                    {snippet}
                  </span>
                </div>
    ...
    return (
      <div className="max-w-[900px]">
        ...
        <div className="flex items-center gap-3 mb-8">
          <h2 className="text-[22px] text-slate-800 dark:text-slate-100 font-normal leading-tight">
        ...
        <div className="space-y-0 pb-16">
          {threadMessages.map((email, index) => {
            ...
            return (
              <div key={email.id || index} className={`pt-2 ${isLast ? 'pb-2' : 'pb-2 border-b border-slate-100 dark:border-[#27272A]'}`}>
                <div className="flex items-start gap-3">
                  {/* Avatar */}
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 text-xl font-medium text-white shadow-sm mt-1 overflow-hidden ${isSent ? 'bg-white dark:bg-slate-900' : 'bg-orange-500'}`}>
                    {isSent ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-1">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-slate-900 dark:text-slate-100 text-[15px]">{senderName}</span>
                        <span className="text-xs text-slate-500 dark:text-slate-400">&lt;{senderEmail}&gt;</span>
                      </div>
                    ...
                    <div className="text-[11px] text-slate-500 dark:text-slate-400 mb-6 flex items-center gap-1 cursor-default">
                      to {receiverName} <svg focusable="false" viewBox="0 0 24 24" className="w-3 h-3 fill-current"><path d="M7 10l5 5 5-5z"></path></svg>
                    </div>

                    <div className="text-sm text-slate-800 dark:text-slate-200 font-sans break-words mb-4 leading-relaxed">
                      <div className="w-full overflow-hidden">
                        <ReactQuill
                          ...
                          className="[&_.ql-container]:!border-none [&_.ql-editor]:text-sm [&_.ql-editor]:font-medium [&_.ql-editor]:text-slate-800 dark:[&_.ql-editor]:text-slate-200 [&_.ql-editor]:!p-0"
                        />
                      </div>
                            ...
                            try {
                              ...
                              return atts.map((att, i) => (
                                <button
                                  ...
                                  className="flex items-center px-3 py-3 border border-slate-200 dark:border-[#27272A] rounded-lg hover:shadow-md cursor-pointer transition bg-white dark:bg-[#1A1A1A] w-fit min-w-[220px] max-w-[280px] group no-underline text-left"
                                >
                      ...
                      {quote && (
                        <div className="mt-3">
                          <button
                            ...
                            className="h-[18px] w-8 bg-slate-100 dark:bg-[#27272A] hover:bg-slate-200 dark:hover:bg-slate-700 rounded flex items-center justify-center text-slate-500 dark:text-slate-400 transition-colors"
                          ...
                          {expandedQuotes[index] && (
                            <div className="mt-3 text-slate-500 dark:text-slate-400 border-l-[3px] border-slate-300 dark:border-[#27272A] pl-3 text-sm">
                              <ReactQuill
                                value={quote || ' '}
                                readOnly={true}
                                theme="snow"
                                modules={{ toolbar: false }}
                                className="[&_.ql-container]:!border-none [&_.ql-editor]:text-sm [&_.ql-editor]:font-medium [&_.ql-editor]:text-slate-500 dark:[&_.ql-editor]:text-slate-400 [&_.ql-editor]:!p-0"
                              />
                            </div>
                          )}
                        </div>
                      ...
                      <div className="mt-6 border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                        ...
                        <div className="bg-white dark:bg-slate-900 p-0">
                          <ReactQuill
                            ...
                            className="w-full bg-white dark:bg-slate-900 [&_.ql-toolbar]:!border-none [&_.ql-toolbar]:!border-b [&_.ql-toolbar]:!border-gray-200 [&_.ql-container]:!border-none [&_.ql-editor]:min-h-[150px] [&_.ql-editor]:text-sm"
                            readOnly={sendingReply}
                          />
                        </div>

                        {/* Attachment Area inside Reply Box */}
                        <div className="px-4 py-4 border-t border-gray-100 bg-white dark:bg-slate-900 flex flex-col gap-3">
                          <label className="block text-xs font-semibold text-slate-700 dark:text-slate-200 mb-0">Attachments</label>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {replyAttachments.map((file, idx) => (
                                <div key={idx} className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 rounded-lg">
                                  ...
                                  <span className="text-xs font-medium text-slate-700 dark:text-slate-200 truncate max-w-[150px]">{file.name}</span>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end overflow-hidden">
      ...
      {/* Side Menu Panel */}
      <div className="relative w-full max-w-[50vw] bg-white dark:bg-slate-900 h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-500 ease-out z-10">
        ...
        >
          {selectedSubject && (
            <button
              ...
              className="p-2 md:p-3 hover:bg-slate-100 rounded-xl transition-all shadow-sm text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 mr-2"
              title="Back to Inbox"
            >
          ...
          <button
            onClick={fetchEmails}
            disabled={loading}
            className="px-3 md:px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-bold rounded-xl hover:bg-slate-50 dark:bg-slate-800 transition-all text-[10px] md:text-xs uppercase tracking-widest flex items-center gap-2 mr-1"
          >
        ...
        {/* Dynamic Content */}
        <div className={`flex-1 overflow-y-auto bg-white dark:bg-slate-900 custom-scrollbar ${selectedSubject ? 'px-6 md:px-8 py-6' : 'px-0 py-2 !pt-0'}`}>
          {loading ? (
      ...
      {previewAttachment && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 sm:p-8 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-2xl overflow-hidden w-full max-w-5xl h-full max-h-[90vh] flex flex-col shadow-2xl relative">
            {/* Header */}
            ...
            <div className="flex-1 bg-gray-100 flex items-center justify-center overflow-auto p-4 md:p-8">
              {previewAttachment.filename.match(/\.(jpg|jpeg|png|gif|webp|svg)$/i) ? (
                ...
                <iframe
                  ...
                  className="w-full h-full rounded-lg shadow-sm bg-white dark:bg-slate-900"
                />

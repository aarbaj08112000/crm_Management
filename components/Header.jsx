...
export default function Header({ isCollapsed, setIsCollapsed, user }) {
  ...
  return (
    <>
      <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-slate-200 dark:border-[#27272A] bg-white dark:bg-[#0A0A0A] px-6 shadow-sm">
        <div className="flex items-center gap-4">
          <button
            onClick={() => setIsCollapsed(!isCollapsed)}
            className="rounded-lg p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 transition-colors cursor-pointer"
          >
            <Menu className="w-6 h-6" />
          </button>
          <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100 dark:text-slate-100 tracking-tight">
            {getPageTitle()}
          </h1>
        </div>
        ...
        <div className="flex items-center gap-6">
          {/* Search Bar */}
          <div className="hidden lg:flex items-center gap-2 bg-slate-50 dark:bg-[#121212] border border-slate-200 dark:border-[#27272A] px-3 py-1.5 rounded-xl text-slate-400 focus-within:border-blue-500 focus-within:bg-white dark:focus-within:bg-[#1A1A1A] transition-all">
            <Search className="w-4 h-4" />
            <input
              type="text"
              placeholder="Search..."
              className="bg-transparent border-none outline-none text-sm text-slate-600 dark:text-slate-300 w-48"
            />
          </div>

          <div className="flex items-center gap-4 border-l border-slate-200 dark:border-[#27272A] pl-6">
            ...
            <div className="relative" ref={notifRef}>
              ...
              {isNotifOpen && (
                <div className="absolute right-0 top-14 w-80 bg-white dark:bg-slate-900 border border-slate-100 rounded-2xl shadow-2xl z-50 animate-in fade-in zoom-in-95 duration-150 overflow-hidden">
                  <div className="px-4 py-3 border-b border-slate-100 dark:border-[#27272A] flex justify-between items-center bg-slate-50 dark:bg-[#121212]">
                    <h3 className="font-semibold text-slate-800 dark:text-slate-100 dark:text-slate-100">Notifications</h3>
                    {notifications.length > 0 && (
                      <button 
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.map(notif => (
                        <div 
                          ...
                          className="px-4 py-3 border-b border-slate-50 hover:bg-slate-50 dark:bg-slate-800 cursor-pointer transition-colors"
                        >
                          <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 dark:text-slate-100">{notif.title}</p>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                <div className="text-right hidden sm:block">
                  <p className="text-sm font-bold text-slate-800 dark:text-slate-100 dark:text-slate-100 leading-none group-hover:text-blue-600 transition-colors">
                    {user?.name || 'Loading...'}
                  </p>
              ...
              {/* Dropdown Menu */}
              {dropdownOpen && (
                <div className="absolute right-0 top-14 w-56 bg-white dark:bg-slate-900 border border-slate-100 rounded-2xl shadow-2xl py-2 z-50 animate-in fade-in zoom-in-95 duration-150">
                  {/* User info in dropdown */}
                  <div className="px-4 py-3 border-b border-slate-100 hidden">
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-100 dark:text-slate-100">{user?.name}</p>
                  ...
                  <div className="py-1">
                    <button
                      ...
                      className="w-full px-4 py-2.5 text-sm text-left font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:bg-slate-800 hover:text-blue-600 flex items-center gap-3 transition-colors cursor-pointer"
                    >

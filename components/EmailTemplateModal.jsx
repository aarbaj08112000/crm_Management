...
import { Save, Loader2, FileText, AlertCircle, Info, UploadCloud, File, Trash2, Paperclip, Eye } from 'lucide-react';
...
export default function EmailTemplateModal({ template, onClose, isViewMode = false }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end overflow-hidden">
      ...
      <div className="relative w-full max-w-3xl bg-white dark:bg-slate-900 h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-500 ease-out">
        ...
        <SidePanelHeader
          icon={FileText}
          title={isViewMode ? "View Template" : (template ? "Edit Template" : "New Template")}
          subtitle={isViewMode ? "View email content and attachments" : "Manage email content and placeholders"}
          onClose={() => onClose()}
        />
        ...
        <div className="flex-1 overflow-y-auto flex flex-col">
          ...
          <div className="flex-1 flex flex-col p-8 min-h-[400px]">
            ...
            <div className="flex-1 rounded-xl border border-slate-200 dark:border-slate-700 focus-within:border-blue-500 transition-all overflow-hidden flex flex-col min-h-0 min-w-0">
              <ReactQuill 
                theme="snow"
                value={body}
                onChange={setBody}
                readOnly={isViewMode}
                modules={isViewMode ? { toolbar: false } : undefined}
                className={cn("flex-1 flex flex-col bg-white dark:bg-slate-900 min-h-[300px] [&_.ql-container]:!border-none [&_.ql-container]:flex-1 [&_.ql-container]:overflow-y-auto [&_.ql-container]:custom-scrollbar [&_.ql-editor]:min-h-full [&_.ql-editor]:text-sm", !isViewMode && "[&_.ql-toolbar]:!border-none [&_.ql-toolbar]:!border-b [&_.ql-toolbar]:!border-slate-200 dark:border-slate-700")}
              />
            </div>
          </div>
          ...
          <div className="px-8 pb-8">
            <div className="flex flex-col gap-4">
              ...
              {/* New Attachments */}
              {!isViewMode && newAttachments.length > 0 && (
                <div className="flex flex-wrap gap-2">
              ...
              {isViewMode && existingAttachments.length === 0 && (
                <div className="text-sm text-slate-500 italic">No attachments for this template.</div>
              )}
            </div>
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="px-8 py-5 bg-white dark:bg-slate-900 border-t border-slate-100 flex items-center justify-end">
      ...
      {viewerOpen && existingAttachments.length > 0 && (
        <AttachmentViewer 
